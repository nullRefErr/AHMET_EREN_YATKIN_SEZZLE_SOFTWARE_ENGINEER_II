...
| `PROMPTS.md` | Record of the prompts used (brief, deliverable 5) | Living record |
...
├── PROMPTS.md                 # brief, deliverable 5
...
| NFR-05 | **Coverage report** | `coverage.out` and HTML can be produced; the README carries the command and the strategy; `domain` ≥ 90% | Deliverable 3 |
...
**On NFR-05:** the brief counts the coverage report as a **deliverable**. A bare percentage is not
enough; it needs a short section explaining which layer is tested and why, plus a report that can
actually be produced. A coverage number alone means nothing — explaining the strategy is what has
value.
...
Mapped one to one against the brief.
...
| 3 | Unit tests and a **coverage report** | `go test -race -coverprofile`, `vitest --coverage`; the command and the strategy in the README | Required |
| 4 | Optional: a Dockerfile running frontend and backend together | Two Dockerfiles + `docker-compose.yml` | Optional |
| 5 | **The prompts used** | `PROMPTS.md` | Required |

> **On deliverable 5:** the plan document never mentions it. The brief says "Share any prompts that
> you used in your work" — that is a **deliverable**, not a courtesy. Prompts are collected in
> `PROMPTS.md` as the work happens; trying to reconstruct them afterwards does not work.

### 6.1 Required README sections

Listed explicitly in item 6 of the brief:
