...
| NFR-05 | **Coverage report** | `COVERAGE.md` is committed and rewritten by `make test`, which is the same command that runs the suites, so it cannot lag behind them; the README explains the strategy; `domain` ≥ 90% | Deliverable 3 |
...
| 3 | Unit tests and a **coverage report** | `COVERAGE.md`, committed and rewritten by `make test` from both suites; the strategy behind it in the README | Required |
