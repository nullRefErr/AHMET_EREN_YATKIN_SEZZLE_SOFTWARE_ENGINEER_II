...
Every setting has a working default, so nothing has to be set to run the project. Copy
`.env.example` to `.env` to change any of them. Anything invalid stops the process at startup rather
...
The backend image is distroless and runs as a non-root user: no shell, no package manager, roughly
15 MB. That also means no `curl` for a Docker health check, so the binary answers the question
...
cd backend && go test -race ./... # race detector: the cache is written concurrently
make cover                       # coverage, text and HTML
```

The strategy matters more than the number, so here is what is tested and why.

| Layer | Coverage | What the tests are for |
|---|---|---|
| `domain` | **97.3%** | Every row of the edge-case table: zero operands, negatives, subnormals, overflow, `NaN`, wrong operand counts. This is where correctness lives, so this is where the tests concentrate |
| `service` | **100%** | Recall versus recompute, failures never stored, and that a broken repository still returns a correct answer |
| `repository/memory` | **91.8%** | Key determinism, commutativity, LRU eviction, recency refresh, and concurrent access under `-race` |
| `transport/rest` | **93.5%** | Status codes and error codes for every failure, the zero-operand case, the `cached` flag over two requests, and that CORS does not reflect an origin it was not given |
| `config` | **100%** | Defaults, parsing, and that bad values stop the process |
| `cmd/server` | 17.6% | Wiring and signal handling. Only the health probe is unit-tested; exercising the rest means running a process, which the other layers already cover |
| **Backend total** | **75.5%** | |
| Front end (reducer) | **100%** | Every state transition, including recovering from an error and continuing from a result |
| Front end (overall) | **84.8%** | Component tests drive the real reducer and client against a stubbed `fetch` |
