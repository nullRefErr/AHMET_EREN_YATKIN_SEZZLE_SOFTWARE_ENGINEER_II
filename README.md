...
generated by `make cover` rather than written by hand, because a figure typed into prose is stale
as soon as the next test lands. `make cover` also writes `backend/coverage.html` to browse line by
line, and CI uploads both profiles as build artifacts on every push.
...
├── COVERAGE.md              # generated by `make cover`, both layers
